...
public abstract class DataflowReadsPipeline extends DataflowCommandLineProgram {
    ...
    final protected void setupPipeline(Pipeline pipeline) {
        ...
        final PCollection<MutableGATKRead> preads = readsDataflowSource.getReadPCollection(intervals, ValidationStringency.SILENT);
    ...
    @VisibleForTesting
    protected PCollection<?> applyTransformsToPipeline(final SAMFileHeader header, final PCollection<MutableGATKRead> preadsIn) {

        PCollection<MutableGATKRead> preads = preadsIn;
    ...
    private static SerializableFunction<MutableGATKRead, Boolean> wrapReadFilter( final ReadFilter filter ) {
        return new SerializableFunction<MutableGATKRead, Boolean>() {
            @Override
            public Boolean apply( MutableGATKRead input ) {
                return filter.apply(input);  // Necessary because we can't use wildcard types for our SerializableFunction
            }
        };
    }

    private static PTransform<? super PCollection<MutableGATKRead>,PCollection<MutableGATKRead>> wrapTransformer(final ReadTransformer transformer, final SAMFileHeader header){
        return ParDo.of(new DataFlowReadFn<MutableGATKRead>(header){
