...
public class OverhangFixingManager {
     ...
     */
    public void addRead(final MutableGATKRead read) {
    ...
    private void fixSplit(final SplitRead read, final Splice splice) {
        ...
        if ( isLeftOverhang(read.loc, splice.loc) ) {
            ...
            if ( overhangingBasesMismatch(read.read.getBases(), 0, splice.reference, splice.reference.length - overhang, overhang) ) {
                final MutableGATKRead clippedRead = ReadClipper.hardClipByReadCoordinates(read.read, 0, overhang - 1);
            ...
            if ( overhangingBasesMismatch(read.read.getBases(), read.read.getLength() - overhang, splice.reference, 0, overhang) ) {
                final MutableGATKRead clippedRead = ReadClipper.hardClipByReadCoordinates(read.read, read.read.getLength() - overhang, read.read.getLength() - 1);
                read.setRead(clippedRead);
            }
        }
    }
    ...
    public final class SplitRead {
        ...
        public MutableGATKRead read;
        public GenomeLoc loc;

        public SplitRead(final MutableGATKRead read) {
            setRead(read);
        }

        public void setRead(final MutableGATKRead read) {
