...
public final class ReadUtils {
    ...
    public static boolean isEmpty( final GATKRead read ) {
        return read.getBases() == null || read.getLength() == 0;
    }

    public static String getBasesString( final GATKRead read ) {
    ...
    public static SAMReadGroupRecord getSAMReadGroupRecordForRead( final GATKRead read, final SAMFileHeader header ) {
        final String readGroupName = read.getReadGroup();
        return readGroupName != null ? header.getReadGroup(readGroupName) : null;
    }

    public static String getPlatformForRead( final GATKRead read, final SAMFileHeader header ) {
    ...
    public static String getPlatformUnitForRead( final GATKRead read, final SAMFileHeader header ) {
        final SAMReadGroupRecord readGroup = getSAMReadGroupRecordForRead(read, header);
        return readGroup != null ? readGroup.getPlatformUnit() : null;
    }

    public static String getLibraryForRead( final GATKRead read, final SAMFileHeader header ) {
        final SAMReadGroupRecord readGroup = getSAMReadGroupRecordForRead(read, header);
        return readGroup != null ? readGroup.getLibrary() : null;
    }

    public static String getSampleNameForRead( final GATKRead read, final SAMFileHeader header ) {
        final SAMReadGroupRecord readGroup = getSAMReadGroupRecordForRead(read, header);
...
//            return false;
     ...
     */
    public static int getMaxReadLength( final List<? extends GATKRead> reads ) {
     ...
     */
    public static MutableGATKRead emptyRead( final MutableGATKRead read ) {
        final MutableGATKRead emptyRead = read.copy();
    ...
    public static void setInsertionBaseQualities( MutableGATKRead read, final byte[] quals) {
        read.setAttribute(BQSR_BASE_INSERTION_QUALITIES, quals == null ? null : SAMUtils.phredToFastq(quals));
    }

    public static void setDeletionBaseQualities( MutableGATKRead read, final byte[] quals) {
     ...
     */
    public static void setBaseQualities( final MutableGATKRead read, final byte[] quals, final EventType errorModel ) {
        switch( errorModel ) {
            case BASE_SUBSTITUTION:
     ...
     */
    public static MutableGATKRead resetOriginalBaseQualities(MutableGATKRead read) {
    ...
    public static boolean alignmentAgreesWithHeader(final SAMFileHeader header, final GATKRead read) {
        ...
        if( referenceIndex == SAMRecord.NO_ALIGNMENT_REFERENCE_INDEX && read.getStart() != ReadConstants.UNSET_POSITION ) {
            return false;
        }
