...
/**
 * A DoFn from Read -> Something which presents a MutableGATKRead to its apply method.
 */
public abstract class DataFlowReadFn<Output> extends DoFn<MutableGATKRead, Output> {
    ...
    /**
     * Subclasses must call this from within their {@link #apply(MutableGATKRead)} call, in order to output results.
    ...
    /**
     * Subclasses must override apply.  It will be called once for every {@link MutableGATKRead} in the input PCollection.
     * Output from apply should be provide by calling {@link #output(Object)}
     * @param read a read to process
     */
    protected abstract void apply(MutableGATKRead read);
}
