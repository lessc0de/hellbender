...
import org.broadinstitute.hellbender.dev.pipelines.bqsr.DataflowReadsFilter;
...
public class BaseRecalibratorDataflow extends DataflowCommandLineProgram {
    ...
    // the inputs to BQSR
    private PCollection<MutableGATKRead> reads;
    ...
    private PCollection<MutableGATKRead> ingestReadsAndGrabHeader(final Pipeline pipeline, List<String> filenames) throws IOException {
        ...
        if (BucketUtils.isCloudStorageUrl(beforePath)) {
            ...
            return new ReadsSource(beforePath, pipeline).getReadPCollection(intervals, ValidationStringency.SILENT)
                    ...
                    .apply(new DataflowReadsFilter(readFilter, header));
        } else {
            ...
            try ( ReadsDataSource readsSource = new ReadsDataSource(new File(beforePath)) ) {
                ...
                List<MutableGATKRead> readLst = new ArrayList<>();
    ...
    private static PCollection<SimpleInterval> ingestKnownIntervals(final Pipeline pipeline, List<FeatureInput<Feature>> knownSites) {
        ...
        for (FeatureInput<Feature> vcfSource : knownSites) {
            ...
            FeatureDataSource<Feature> source = new FeatureDataSource<>(featureFile, (FeatureCodec<Feature, ?>)FeatureManager.getCodecForFile(featureFile), "KnownIntervals");
