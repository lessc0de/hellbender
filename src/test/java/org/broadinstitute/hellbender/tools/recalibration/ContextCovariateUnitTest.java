...
public final class ContextCovariateUnitTest extends BaseTest {
    ...
    public void testSimpleContexts() {
        ...
        for(int i = 0; i < 10; i++) {
            final MutableGATKRead read = ArtificialReadUtils.createRandomRead(header, 1000);
            read.setIsReverseStrand(rnd.nextBoolean());
            final MutableGATKRead clippedRead = ReadClipper.clipLowQualEnds(read, RAC.LOW_QUAL_TAIL, ClippingRepresentation.WRITE_NS);
    ...
    public static void verifyCovariateArray(int[][] values, int contextSize, MutableGATKRead read, Covariate contextCovariate, final byte lowQualTail) {
    ...
    public void testStrandedBytes(final String baseStr, final byte[] quals, final String cigar, final boolean neg, final int lowQTail, final String expecteBaseStr){
        ...
        final MutableGATKRead read = ArtificialReadUtils.createArtificialRead(bases, quals, cigar);
    ...
    public static String expectedContext (final MutableGATKRead originalRead, final int offset, final int contextSize, final byte lowQualTail) {
