...
public final class CycleCovariateUnitTest extends BaseTest {
    ...
    public void testSimpleCycles() {
        ...
        short readLength = 10;
        MutableGATKRead read = ArtificialReadUtils.createRandomRead(header, readLength);
    ...
    public void testMoreThanMaxCycleFails() {
        ...
        MutableGATKRead read = ArtificialReadUtils.createRandomRead(readLength);
        read.setIsPaired(true);
        read.setReadGroup(illuminaReadGroup.getReadGroupId());
    ...
    public static int expectedCycle(MutableGATKRead read, final int baseNumber, final boolean indel, final int maxCycle) {
    ...
    public void testCycleKey(final String cigar, final boolean isNegStrand, final boolean isSecondInPair, final int baseNumber, final int maxCycle, final boolean indel, final int expectedCycleKey){
        final MutableGATKRead read = ArtificialReadUtils.createArtificialRead(TextCigarCodec.decode(cigar));
        read.setIsReverseStrand(isNegStrand);
        read.setIsPaired(isSecondInPair);
        read.setIsSecondOfPair(isSecondInPair);
