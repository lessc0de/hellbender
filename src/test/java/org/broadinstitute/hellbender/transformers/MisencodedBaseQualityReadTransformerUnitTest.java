...
public final class MisencodedBaseQualityReadTransformerUnitTest extends BaseTest {
    ...
    private MutableGATKRead createRead(final byte[] quals) {
        final String readBases = "AAAAAAAAAA";
        MutableGATKRead read = ArtificialReadUtils.createArtificialRead(header, "foo", 0, 10, readBases.getBytes(), quals);
        read.setCigar("10M");
        return read;
    }
    ...
    public void testGoodQuals() {
        ...
        MutableGATKRead read = createRead(goodQuals);
        MutableGATKRead newRead = tr.apply(read);
    ...
    public void testFixBadQuals() {
        ...
        final ReadTransformer tr = new MisencodedBaseQualityReadTransformer();
        final MutableGATKRead read = createRead(badQuals);
        final MutableGATKRead fixedRead = tr.apply(read);
    ...
    public void testFixGoodQualsBlowUp() {
        final byte[] fixedQuals = { 28, 29, 31, 32, 33, 30, 31, 27, 26, 25 };
        final ReadTransformer tr = new MisencodedBaseQualityReadTransformer();
        final MutableGATKRead read = createRead(fixedQuals);
        tr.apply(read);
    }
}