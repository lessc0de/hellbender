...
public final class ArtificialReadUtilsUnitTest extends BaseTest {
    ...
    public void basicReadIteratorTest() {
        Iterator<MutableGATKRead> iter = ArtificialReadUtils.mappedReadIterator(1, 100, 100);
        int count = 0;
        while (iter.hasNext()) {
            MutableGATKRead rec = iter.next();
            count++;
        }
    ...
    public void basicUnmappedIteratorTest() {
        Iterator<MutableGATKRead> iter = ArtificialReadUtils.mappedAndUnmappedReadIterator(1, 100, 100, 1000);
        int count = 0;
