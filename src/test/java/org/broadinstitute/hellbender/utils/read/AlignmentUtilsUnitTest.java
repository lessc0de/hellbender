...
public final class AlignmentUtilsUnitTest {
    ...
    private MutableGATKRead createMappedRead(String name, int start) {
    ...
    public void testNumAlignedBasesCountingSoftClips(final Cigar cigar, final int expected) {
        final MutableGATKRead read = ArtificialReadUtils.createArtificialRead(header, "myRead", 0, 1, cigar == null ? 10 : cigar.getReadLength());
        read.setCigar(cigar);
    ...
    public void testSoftClipsData(final byte[] qualsOfSoftClipsOnLeft, final int middleSize, final String middleOp, final byte[] qualOfSoftClipsOnRight, final int qualThreshold, final int numExpected) {
        ...
        final MutableGATKRead read = ArtificialReadUtils.createArtificialRead(header, "myRead", 0, 1, readLength);
    ...
    public Object[][] makeMismatchCountDataProvider() {
                    ...
                    for ( final int lengthOfFirstM : Arrays.asList(0, 3) ) {
                        for ( final char middleOp : Arrays.asList('M', 'D', 'I') ) {
                            for ( final int mismatchLocation : Arrays.asList(-1, 0, 5, 10, 15, 19) ) {
                                ...
                                final MutableGATKRead read = ArtificialReadUtils.createArtificialRead(header, "myRead", 0, locationOnReference, readLength);
        ...
        MutableGATKRead read = ArtificialReadUtils.createArtificialRead(header, "myRead", 0, 10, 20);
    ...
    public void testMismatchCountData(final MutableGATKRead read, final int refIndex, final int startOnRead, final int basesToRead, final boolean isMismatch) {
