...
public final class ReadUtilsUnitTest extends BaseTest {
    private interface GetAdaptorFunc {
        public int getAdaptor(final MutableGATKRead record);
    }
    ...
    public Object[][] makeActiveRegionCutTests() {
        ...
        tests.add( new Object[]{ new GetAdaptorFunc() {
            @Override public int getAdaptor(final MutableGATKRead record) { return ReadUtils.getAdaptorBoundary(record); }
        }});
    ...
    private MutableGATKRead makeRead(final int fragmentSize, final int mateStart) {
        final byte[] bases = {'A', 'C', 'G', 'T', 'A', 'C', 'G', 'T'};
        final byte[] quals = {30, 30, 30, 30, 30, 30, 30, 30};
        final String cigar = "8M";
        MutableGATKRead read = ArtificialReadUtils.createArtificialRead(bases, quals, cigar);
    ...
    public void testGetBasesReverseComplement() {
        ...
        while(iterations-- > 0) {
            ...
            MutableGATKRead read = ArtificialReadUtils.createRandomRead(l);
    ...
    public void testGetMaxReadLength() {
        for( final int minLength : Arrays.asList( 5, 30, 50 ) ) {
            for( final int maxLength : Arrays.asList( 50, 75, 100 ) ) {
                final List<MutableGATKRead> reads = new ArrayList<>();
        ...
        final List<MutableGATKRead> reads = new LinkedList<>();
    ...
    public void testReadWithNsRefIndexInDeletion() throws FileNotFoundException {
        ...
        final MutableGATKRead read = ArtificialReadUtils.createArtificialRead(header, "myRead", 0, 8975, readLength);
    ...
    public Object[][] makeHasWellDefinedFragmentSizeData() throws Exception {
        ...
        final MutableGATKRead read = ArtificialReadUtils.createArtificialRead(header, "read1", 0, 10, 10);
        read.setIsPaired(true);
        read.setIsProperlyPaired(true);
        ...
        {
            final MutableGATKRead bad1 = read.copy();
            bad1.setIsPaired(false);
            tests.add( new Object[]{ "not paired", bad1, false });
        }

        {
            final MutableGATKRead bad = read.copy();
...
//            tests.add( new Object[]{ "not proper pair", bad, false });
    ...
    private void testHasWellDefinedFragmentSize(final String name, final MutableGATKRead read, final boolean expected) {
