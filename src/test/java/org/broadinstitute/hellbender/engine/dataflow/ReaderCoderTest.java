...
public final class ReaderCoderTest {
    ...
    public Object[][] uuids(){
        ...
        List<MutableGATKRead> googleReads;// = Lists.newArrayList(makeGoogleRead(1, 10, 1), makeGoogleRead(20, 40, 2));
        List<MutableGATKRead> samReads = Lists.newArrayList(makeSamRead(1, 10, 1), makeSamRead(20, 40, 2));
        /*
    ...
    private MutableGATKRead makeGoogleRead(int start, int length, int i) {
        return ArtificialReadUtils.createRandomGoogleRead(start, length, i);
    }

    public MutableGATKRead makeSamRead(int start, int length, int i) {
        return ArtificialReadUtils.createRandomRead(start, length, i);
    }

    @Test(dataProvider = "reads")
    public void createUuidsTest(List<MutableGATKRead> samReads/*, List<MutableGATKRead> googleReads*/) {
        ...
        //p.getCoderRegistry().registerCoder(MutableGATKRead.class, ReadCoder.CODER);
        ...
        PCollection<MutableGATKRead> pShards = p.apply(Create.of(samReads));

        List<MutableGATKRead> sameReads = Lists.newArrayList();
