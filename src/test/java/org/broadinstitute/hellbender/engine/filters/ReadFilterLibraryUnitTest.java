...
public final class ReadFilterLibraryUnitTest {
     ...
     */
    private MutableGATKRead createRead( final SAMFileHeader header, final Cigar cigar, final int group, final int reference, final int start ) {
        final MutableGATKRead record = ArtificialReadUtils.createArtificialRead(header, cigar);
    ...
    private MutableGATKRead createRead( final SAMFileHeader header, final String cigarString ) {
        final Cigar cigar = TextCigarCodec.decode(cigarString);
        return createRead(header, cigar, 1, 0, 10);
    }

    private MutableGATKRead simpleGoodRead( final SAMFileHeader header ) {
        final String cigarString = "101M";
        final Cigar cigar = TextCigarCodec.decode(cigarString);
        MutableGATKRead read = createRead(header, cigar, 1, 0, 10);
    ...
    public void testCheckSeqStored() {
        final MutableGATKRead goodRead = ArtificialReadUtils.createArtificialRead(new byte[]{(byte) 'A'}, new byte[]{(byte) 'A'}, "1M");
        final MutableGATKRead badRead = ArtificialReadUtils.createArtificialRead(new byte[]{}, new byte[]{}, "1M");
    ...
    public void testCigarNOperatorFilter(String cigarString) {
        ...
        final MutableGATKRead read = createRead(header, cigarString);
    ...
    public void passesAllFilters() {
        final SAMFileHeader header = createHeaderWithReadGroups();
        final MutableGATKRead read = simpleGoodRead(header);
    ...
    public void VALID_ALIGNMENT_START_allows_unmapped_case2() {
        ...
        MutableGATKRead readWithUnmappedPosition = new SAMRecordToGATKReadAdapter(samWithUnmappedPosition);
    ...
    public void testAlignmentAgreesWithHeader() {
        ...
        // SAMRecords with contigs not found in the header will automatically get marked as unmapped,
        // and so will pass the AlignmentAgreesWithHeader filter.
        //final Read googleRead = read.convertToGoogleGenomicsRead();
        //googleRead.getAlignment().getPosition().setReferenceName("BAD_CONTIG");
        //final MutableGATKRead badContigRead = new GoogleGenomicsReadToGATKReadAdapter(googleRead);
        //Assert.assertFalse(filter.test(badContigRead), "AlignmentAgreesWithHeader read filter should have failed on read with bad contig");
    ...
    public void testMaxInsertSizeFilter() {
        final SAMFileHeader header = createHeaderWithReadGroups();
        final MutableGATKRead pairedRead = simpleGoodRead(header);
        final MutableGATKRead unpairedRead = simpleGoodRead(header);
