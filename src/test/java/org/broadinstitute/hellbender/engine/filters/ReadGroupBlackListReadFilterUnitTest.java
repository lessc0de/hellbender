...
public final class ReadGroupBlackListReadFilterUnitTest extends BaseTest {
    ...
    public void testFilterReadGroup() throws IOException {
        MutableGATKRead filteredRecord = ArtificialReadUtils.createArtificialRead(header, "readUno", 0, 1, 20);
        filteredRecord.setReadGroup(getReadGroupId(1));

        MutableGATKRead unfilteredRecord = ArtificialReadUtils.createArtificialRead(header, "readDos", 0, 2, 20);
    ...
    public void testFilterOutByReadGroup() throws IOException {
        ...
        List<MutableGATKRead> records = new ArrayList<>();
        ...
        for (int x = 0; x < GROUP_COUNT; x++) {
            ...
            for (int y = 1; y <= recordsPerGroup; y++) {
                MutableGATKRead record = ArtificialReadUtils.createArtificialRead(header, "readUno", 0, ++alignmentStart, 20);
        ...
        int filtered = 0;
        int unfiltered = 0;
        for (MutableGATKRead record : records) {
    ...
    public void testFilterOutByAttribute() throws IOException {
        ...
        int unfiltered = 0;
        for (MutableGATKRead record : records) {
            String platformUnit = ReadUtils.getPlatformUnitForRead(record, header);
            if (!filter.test(record)) {
